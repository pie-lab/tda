...
The data can be downloaded [here](https://doi.org/10.7910/DVN/5T80PF). NOTE that the csv version of this file will mis-read two adjectives when opened in MS Excel: "false" and "blasé". These must be fixed manually prior to working with the data in Excel (but it appears to work as expected in R).
...
# data_re = filter(data_re, included == "Yes") # remove participants screened out for demographic and quality reasons
