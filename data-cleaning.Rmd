...
First we create a vector for each item that specifies the new labels. All distractors take the form `D1` through `D5`. `Ans` (for the correct answer) is then inserted in the location of the correct response. We use `IDK` to represent "I don't know" and `None` to represent "None of these."
...
masterKey = masterKey %>%
  mutate(relevel = map(Key, 
                       ~insert(c("D1","D2","D3","D4","D5","IDK", "None"), 
                              at = .x, "Ans")))
```
